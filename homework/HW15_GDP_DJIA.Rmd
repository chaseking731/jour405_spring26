...
```{r}
GDP_DJIA_data |> summarize(correlation = cor(DJIA, GDP, method="pearson"))
...
GDP_DJIA_data |> 
  ...
  labs(title = "Correlation between DJIA and GDP")
...
There is a very strong connection between DJIA and GDP. The was a 95% correlation between the two which means that one very likely would effect the other. The line representing the correlation between DJIA and GDP is almost at a 45 degree angle on the scatterplot which would mean that it is close to a perfect correlation. We can see that as the DJIA and GDP grow that they start to stray from that line.
