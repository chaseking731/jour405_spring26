...
The range is from 0.23 to 3.04.

2. What does it mean when a respondent has a weight of 0.23? What about 3.04?

A weight of 0.23 means that a respondents answers weigh 23% of a typical answer and a weight of 3.04 means that a respondents answers weigh 304% of a typical answer. 

3. Why might some respondents be weighted more heavily than others?
It is to account for sampling inbalances. If somebody is from an underrepsented race or gender there answers will weigh more than if somebody is from an overrepresented race or gender.
...
The 'rarely/never' category saw the biggest difference between weighted and unweighted results.

2. How might these differences affect reporting on likely voter turnout?
It gives more context surronding the voter categories. Like with unweighted results it looks like it over represents the data and weighting it can put better expectations.
...
Because a lot of people that rarely/never vote are less likely to engage in the sample. So the sample might see higher expected turnouts before being weighted. 
