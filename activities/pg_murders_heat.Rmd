...
```{r}
pg_murders |> summarize(correlation = cor(temperature, murders, method="pearson"))
```
...
There is a meaningfully positive correlation between warmer temperature and the amount of murders. The correlation calculation is 0.5825525, which shows that there is a 58.25% correlation between the tempature and murders. On the scatterplot the line shows a positive correlation and that there are likely to have more murders when the tempature is higher. The scatterplot also shows that August has a high tempature and less murders, indicating that it is not fully correlated.
