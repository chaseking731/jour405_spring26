...
I would say that the model is useful because using all of the data it can predict the wins within a reasonable amount. It is statistically significant and the deviation of 2.072 isn't necessarily huge. 62.32$ of the wins being explained by the model is pretty good, especially with how random sports can be. There are so many variables that go into each game, so using a couple of variables to have a statistically significant prediction is useful.
...
Points scored has the biggest impact on wins and Yards Gained does not seem very impactful and does not do much for the data.
...
I could use this model to explain how much impact a multi-interception game from a certain quarterback had against winning by explaining the correlation between giveaways and winning. There are a lot of other ways you can use it by explaining how allowing yards or allowing a certain amount of points would hurt the teams chances of winning as well. 